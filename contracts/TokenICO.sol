...
contract TokenICO {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "Only contract owner can perform changes");
        _;
    }
    ...
    function buyToken(uint256 _tokenAmount) public payable {
        require(
            ...
            "Insufficient Amount provided for the token purchase"
        );

        ERC20 token = ERC20(tokenAddress);
        require(
            _tokenAmount < token.balanceOf(address(this)),
            "Insufficient Amount provided for the purchase"
        );
